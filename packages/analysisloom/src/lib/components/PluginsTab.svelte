<script>
  ...
  async function loadPlugins() {
    ...
    try {
      ...
      msg = `✅ ${plugins.length} plugins available`;
    } catch (e) {
  ...
  async function runPlugin() {
    ...
    try {
      ...
      msg = runResult.success ? `✅ ${selected.name} completed` : `❌ ${runResult.error}`;
    } catch (e) {
      msg = `❌ ${typeof e === "string" ? e : String(e)}`;
    }
    busy = false;
...
<div class="panel">
  <h3>Plugin SDK</h3>
  <p class="hint">Built-in forensic plugins — extensible Rust trait interface</p>
  ...
  {#if selected}
    <div class="run-panel">
      <h4>Run: {selected.name}</h4>
      <div class="row">
        <input type="text" bind:value={filePath} placeholder="Target file path" disabled={busy} />
        <button onclick={runPlugin} disabled={busy} class="btn-primary">Run Plugin</button>
      </div>
      {#if runResult}
        <pre class="output">{JSON.stringify(runResult.output, null, 2)}</pre>
      {/if}
    </div>
  {/if}
</div>
