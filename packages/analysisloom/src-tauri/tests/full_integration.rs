...
fn full_forensic_pipeline_with_random_fixtures() {
    ...
    let windows = scan_windows_artifacts(ws.root.to_string_lossy().to_string())
        .expect("scan_windows_artifacts");
    assert!(
        windows.prefetch_count >= 1,
        "Should find synthetic prefetch"
    );
    assert!(windows.lnk_count >= 1, "Should find synthetic LNK");
    println!(
        "Windows artifacts: prefetch={}, lnk={}",
        windows.prefetch_count, windows.lnk_count
    );

    let stego = scan_steganography(vec![ws
        .root
        .join("stego_sample.png")
        .to_string_lossy()
        .to_string()])
    ...
    let email =
        scan_email_directory(ws.root.to_string_lossy().to_string()).expect("scan_email_directory");
    ...
    let chat =
        scan_chat_artifacts(ws.root.to_string_lossy().to_string()).expect("scan_chat_artifacts");
