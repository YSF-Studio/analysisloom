...
pub async fn start_carving(image_path: String, output_dir: String) -> Result<(), String> {
    ...
    if let Ok(mut progress) = forensic::PROGRESS_STATE.lock() {
...
<body>
  ...
  <div class="footer">
    ...
    Tool limitations and hash validation are documented in the report notes.
  </div>
</body></html>"#,
        name = case.name,
        id = case.id,
